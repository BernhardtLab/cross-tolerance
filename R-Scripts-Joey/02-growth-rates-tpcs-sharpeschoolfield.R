...
df <- d  
fit_model_ss <- function(df, model_name = "sharpeschoolhigh_1981", iter = 2000) {
  ...
  # construct formula, handling tref parameter if needed
...
fits_ss <- df %>%
  group_by(curve_id) %>%
  nest() %>%
  mutate(fit = map(data, ~ fit_model_ss(.x, iter = 2000)))

fit_status_ss <- fits_ss %>%
...
preds2 <- preds_ss %>% 
  ...
  ggplot() +
    ...
    labs(
      x = "Temperature (°C)",
      y = "Growth rate",
      title = "SS fits per population"
    )
  ggsave("figures/tpcs-all-no-lag-ss.png", width = 10, height = 6)
...
write_csv(p3, "data-processed/all-SS-traits-new.csv")
p3b <- read_csv("data-processed/all-SS-traits.csv") |> 
  select(-temp_20pct)

waldo::compare(p3, p3b) ### note this is me checking that the SS outputs are the same as the pre-spring cleaning -- they are!
