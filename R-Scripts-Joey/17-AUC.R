...
# Created: June 2026
#
# Estimates AUC from OD curves using gcplyr (model-free, moving-average
# smoothing), then fits Sharpe-Schoolfield High (SSH) thermal performance
# curves per strain using AUC as the performance metric.
#
#
# Input:  data-processed/all-blocks-tpc-experiment.csv
# Output: data-processed/gcplyr/     (suffix: -17)
#         figures/                   (suffix: -17)
#
# Sections:
#   1. Setup
...
# ~15-min data resolution; window of 5 ≈ 75-min smoothing
SMOOTH_N <- 5
...
# Helper: Welch two-sample or one-sample t-test + Wilcoxon, returns tidy row
run_tests <- function(x, y = NULL, mu = NULL, label_x, label_y = "ancestor", trait) {
  if (!is.null(y)) {
...
results <- bind_rows(
  run_tests(g35$topt, g40$topt,      label_x = "35 evolved", label_y = "40 evolved", trait = "topt"),
  run_tests(g35$topt, mu = anc_topt, label_x = "35 evolved",                         trait = "topt"),
...
run_auc_tests <- function(data, trait_col, trait_label, anc_val) {
  ...
  bind_rows(
    run_tests(g35, g40,          label_x = "35 evolved", label_y = "40 evolved", trait = trait_label),
    run_tests(g35, mu = anc_val, label_x = "35 evolved",                         trait = trait_label),
    run_tests(g40, mu = anc_val, label_x = "40 evolved",                         trait = trait_label)
  ) |>
    mutate(
      p_welch_holm  = p.adjust(p_welch,  method = "holm"),
      p_wilcox_holm = p.adjust(p_wilcox, method = "holm")
...
ggsave(file.path(FIGS, "tpc-per-strain-gcplyr-17.pdf"), p_per_strain,
       width = 16, height = 18)
cat("Saved: tpc-per-strain-gcplyr-17.pdf\n")


# ── Per-well AUC diagnostic PDF ───────────────────────────────────────────────
#
# Raw blank-corrected OD (grey points), smoothed OD curve (colored line),
# and AUC region shaded. Shows exactly what is being integrated.
...
pdf(file.path(FIGS, "auc-per-well-gcplyr-17.pdf"), width = 16, height = 12)
...
dev.off()
cat(sprintf("Saved: auc-per-well-gcplyr-17.pdf (%d pages)\n", auc_n_pages))
...
write_csv(gcplyr_metrics,  file.path(OUT, "gcplyr-metrics-per-well-17.csv"))
write_csv(tpc_params_auc,  file.path(OUT, "tpc-params-auc-gcplyr-17.csv"))
write_csv(tpc_preds_auc,   file.path(OUT, "tpc-predictions-auc-gcplyr-17.csv"))
write_csv(results,         file.path(OUT, "stats-results-auc-gcplyr-17.csv"))
write_csv(results_pred,    file.path(OUT, "stats-results-auc-41-42-predicted-17.csv"))
write_csv(results_obs,     file.path(OUT, "stats-results-auc-41-42-observed-17.csv"))
