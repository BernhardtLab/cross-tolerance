...
#
# Identical to nglabratus_tpc_analysis.R except sections 1 & 2:
# OD data is imported from data-processed/all-blocks-tpc-experiment.csv
# instead of Table_S1.xlsx.
#
# Input:  data-processed/all-blocks-tpc-experiment.csv
# Output: Processed_Outputs_raw_import/
#
# Growth rates estimated using sliding-window max slope on ln(OD) only —
# no 3-parameter logistic fitting.
# =============================================================================
...
OUT        <- "R-Script-Drafts/TPC-Analysis/Processed_Outputs_raw_import"
...
# =============================================================================
# 4. Estimate growth rates (sliding-window max slope on ln(OD))
# =============================================================================

fit_growth_sliding_window <- function(time_h, OD, min_points = 6) {
  n <- length(time_h)
...
gr_list <- df_all %>%
  ...
  group_map(function(grp, keys) {
    res <- fit_growth_sliding_window(grp$time_h, grp$OD)
