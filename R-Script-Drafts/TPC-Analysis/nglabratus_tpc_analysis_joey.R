...
fit_growth_logistic <- function(time_h, OD, min_points = 6) {
  ...
  n <- length(time_h)
  if (n < min_points) return(c(mu = NA_real_, r2 = NA_real_, lag_h = NA_real_, K = NA_real_, r = NA_real_, t_mid = NA_real_))
  ...
  result <- tryCatch({
    ...
    # mu (day⁻¹), R², lag (h), and fitted parameters K, r, t_mid
  ...
  c(mu = best_mu, r2 = best_r2, lag_h = NA_real_, K = NA_real_, r = NA_real_, t_mid = NA_real_)
}
...
# =============================================================================
# 4b. Plot fitted logistic curves on raw data
# =============================================================================

# Define logistic function for predictions
logistic3 <- function(t, K, r, t_mid) {
  K / (1 + exp(-r * (t - t_mid)))
}

# Create output folder for logistic fits
...
for (i in seq_len(nrow(plot_wells))) {
  ...
  od_pred <- logistic3(time_pred, K_fit, r_fit, t_mid_fit)
